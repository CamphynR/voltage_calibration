...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(6, 4, figsize = (32, 24), sharex = True, sharey = True) #, layout = "constrained")
    axs = np.ndarray.flatten(axs)
    for c in range(24):
        axs[c].plot(adc_diff[:, c],
                    color = "blue", label = r"mean | diff (vc curve) | ")
        axs[c].fill_between(range(len(adc_diff[:, c])), adc_diff[:, c],
                            adc_diff[:, c] - 0.5 * adc_diff_std[:, c],
                            adc_diff[:, c] + 0.5 * adc_diff_std[:, c],
                            color = "blue")
        # line to indicate 1 jan 2023
        idx = find_2023_time_idx(times)
        axs[c].vlines(idx, 0, np.max(adc_diff[:, :]),
                      ls = "dashed", color = "red", label = "2022")
        
        axs[c].legend(loc = "best", fontsize = "xx-large")
        tick_step = int(len(times_utc)/10)
        axs[c].set_xticks(range(len(times_utc))[::tick_step], times_utc[::tick_step], rotation = -45, size = "xx-large")
        axs[c].tick_params(axis = "y", labelsize = "xx-large")
        axs[c].grid()
        axs[c].set_title(f"channel {c}", size = 24)

        if args.exclude_2022:
            axs[c].set_xlim(idx, None)
            axs[c].set_ylim(None, 1.1 * np.max(adc_diff[idx + 1:,:]))

        if (c - 3.) %    4. == 0:
            ax2 = axs[c].secondary_yaxis("right",
